...
struct OSMConnectionEdge {
  ...
  OSMConnectionEdge(GraphId startnode,
                    ...
                    std::list<PointLL> startshape,
                    std::list<PointLL> endshape,
                    const PointLL& bss_ll): startnode(startnode), endnode(endnode),
                        wayid(wayid), names(std::move(names)), startshape(std::move(startshape)), endshape(std::move(endshape)),
                        bss_ll(bss_ll)  {}
...
std::vector<OSMConnectionEdge> project(const GraphTile& local_tile, const std::vector<OSMNode>& osm_bss)  {

  std::vector<OSMConnectionEdge> res;
  auto local_level = TileHierarchy::levels().rbegin()->first;
  // In this loop, we try to find the way on which to project the bss node by iterating all nodes in its
  // corresponding tile... Not a good idea in term of performance... any better idea???
  for(const auto& bss : osm_bss) {
    const PointLL bss_ll(bss.latlng());
    ...
    // Create a temporary connection which starts from a existing way node in the tile and point to the bss node
    {
      std::list<PointLL> startshape;
      ...
      res.emplace_back(startnode, endnode, way_id, std::move(names), std::move(startshape),std::move(endshape), bss_ll);
    }
  }
  boost::sort(res);
  return res;
...
void create_bss_node_and_edges(GraphTileBuilder& tilebuilder_local,
  ...
  auto _finally = make_finally([&tilebuilder_local, &tile, &lock](){
    LOG_INFO("Storing local tile data with bss nodes, tile id: " + std::to_string(tile.id().tileid()));
  ...
  for (auto& nb : currentnodes) {
    ...
    // If the node should be a part of bss's edge then create the edge and update the node's edge_count
    for(auto& conn : new_connections) {
      ...
      if (conn.startnode.id() == nodeid) {
        ...
        auto directededge = make_directed_edge({},
                                               conn.startshape,
  ...
  for(const auto& conn : new_connections) {
    ...
    GraphId new_bss_node_graphid{tile.header()->graphid().tileid(), local_level, static_cast<uint32_t>(tilebuilder_local.nodes().size())};
    ...
    //update startnode -> bssnode
    {
      ...
      bool added;
      uint32_t edge_info_offset = tilebuilder_local.AddEdgeInfo(conn.start_to_bss_edge_idx,
                                                                conn.startnode, new_bss_node_graphid, conn.wayid, 0, 0, 0, conn.startshape,
                                                                conn.names, 0, added);
    ...
    //update endnode -> bssnode
    {
      ...
      directededge.set_endnode(new_bss_node_graphid);
      bool added;
      uint32_t edge_info_offset = tilebuilder_local.AddEdgeInfo(conn.end_to_bss_edge_idx,
                                                                conn.endnode, new_bss_node_graphid, conn.wayid, 0, 0, 0, conn.endshape,
                                                                conn.names, 0, added);
      directededge.set_edgeinfo_offset(edge_info_offset);
    }
    ...
    {
      auto directededge = make_directed_edge(conn.startnode,
                                             conn.startshape,
                                             0,
                                             conn.names.size());
      bool added;
      uint32_t edge_info_offset = tilebuilder_local.AddEdgeInfo(conn.start_to_bss_edge_idx,
                                                                new_bss_node_graphid, conn.startnode, conn.wayid, 0, 0, 0, conn.startshape,
                                                                conn.names, 0, added);
      directededge.set_edgeinfo_offset(edge_info_offset);
      tilebuilder_local.directededges().emplace_back(std::move(directededge));
      ++added_edges;
    ...
    {
      auto directededge = make_directed_edge(conn.endnode,
                                             conn.endshape,
                                             1,
                                             conn.names.size());
      bool added;
      uint32_t edge_info_offset = tilebuilder_local.AddEdgeInfo(conn.end_to_bss_edge_idx,
                                                                new_bss_node_graphid, conn.endnode, conn.wayid, 0, 0, 0, conn.endshape,
                                                                conn.names, 0, added);
      directededge.set_edgeinfo_offset(edge_info_offset);
      tilebuilder_local.directededges().emplace_back(std::move(directededge));
      ++added_edges;
  ...
  LOG_INFO(std::string("Added: ") + std::to_string(added_edges) + " edges and " + std::to_string(added_nodes) + " bss nodes");
}
...
void BssBuilder::Build(const boost::property_tree::ptree& pt,
  ...
  for (const auto& node: osm_nodes) {
    auto tile_id = TileHierarchy::GetGraphId({node.latlng()}, local_level);
  ...
  size_t nb_threads = std::max(static_cast<uint32_t>(1),
  ...
  LOG_INFO("Adding " + std::to_string(osm_nodes.size()) + " bike share stations to " +
           std::to_string(bss_by_tile.size()) + " local graphs with " + std::to_string(nb_threads) + " thread(s)");
  ...
  for (size_t i = 0; i < threads.size(); ++i) {
    ...
    // Make the thread
    threads[i].reset(new std::thread(build, std::cref(pt.get_child("mjolnir")),
