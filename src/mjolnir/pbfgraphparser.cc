...
public:
  ...
  virtual void way_callback(uint64_t osmid,
    ...
    for (const auto& tag : results) {
      ...
      } else if (tag.first == "guidance_view:jct:base" || tag.first == "guidance_view:jct:base:forward") {
        w.set_fwd_jct_base_index(osmdata_.name_offset_map.index(tag.second));
      } else if (tag.first == "guidance_view:jct:overlay" || tag.first == "guidance_view:jct:overlay:forward") {
