...
void update_valhalla_tiles(const bpt::ptree& pt,
  ...
  for (; tile_start != tile_end; ++tile_start) {
    ...
    for (const auto& speeds : tile_start->second) {
      // Go through directed edges and update the speeds
...
int main(int argc, char** argv) {
  ...
  LOG_INFO("Update " + std::to_string(updated_count) + " directed edges.");
  LOG_INFO("Finished");
...
void testAddPredictedTraffic() {

}
} // namespace

int main() {
  test::suite suite("predicted_traffic");

  suite.test(TEST_CASE(testAddPredictedTraffic));
