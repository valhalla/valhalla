...
namespace one_stop {
  ...
  std::vector<bool> float_to_bits(float value,float lower=-90.0, float middle=0.0, float upper=90.0,int length=15 ) {
...
void save_nodes(Transit& tile,
  ...
  gtfs::Stops stops;
  std::copy_if(feed.get_stops().begin(), feed.get_stops().end(), std::back_inserter(stops), [bbox](gtfs::Stop stop){return bbox.Contains({stop.stop_lon, stop.stop_lat});});
  ...
  */
  ...
  for(const auto& station: stations) {
    ...
    for(const auto& egress_pt: station.second->egress ) {
      ...
      node->set_onestop_id(one_stop::generate_one_stop_id(one_stop::OneStopIdType::Stop, egress_pt.stop_name, { egress_pt.stop_lon, egress_pt.stop_lat }));
      ...
      if(egress_pt.wheelchair_boarding.size()== 0 || egress_pt.wheelchair_boarding == "0") {
        if(station.second->station.wheelchair_boarding.size() ==0 ||station.second->station.wheelchair_boarding == "0" ) {
    ...
    node->set_onestop_id(one_stop::generate_one_stop_id(one_stop::OneStopIdType::Stop, stationNode.stop_name, { stationNode.stop_lon, stationNode.stop_lat }));
    ...
    for( const auto &platforms_pt: station.second->platform) {
      ...
      node->set_onestop_id(one_stop::generate_one_stop_id(one_stop::OneStopIdType::Stop, platforms_pt.stop_name, { platforms_pt.stop_lon, platforms_pt.stop_lat }));
...
void save_route(Transit& tile, 
  ...
  for(auto& route_pt:  routes) {
    ...
    route->set_onestop_id(one_stop::generate_one_stop_id(one_stop::OneStopIdType::Route, route_pt.route_long_name));
    ...
    if (vehicle_type == gtfs::RouteType::Tram || vehicle_type == gtfs::RouteType::TramService) {
      ...
      type = Transit_VehicleType::Transit_VehicleType_kRail;
               ...
               vehicle_type == gtfs::RouteType::ExpressBusService || vehicle_type == gtfs::RouteType::LocalBusService ||
               vehicle_type == gtfs::RouteType::BusService || vehicle_type == gtfs::RouteType::ShuttleBus ||
               vehicle_type == gtfs::RouteType::DemandAndResponseBusService ||
               vehicle_type == gtfs::RouteType::RegionalBusService || vehicle_type == gtfs::RouteType::CoachService) {
    ...
    route->set_operated_by_onestop_id(one_stop::generate_one_stop_id(one_stop::OneStopIdType::Operator, (*agency).agency_name));
    ...
    std::string route_text_color = (route_pt.route_text_color == "" ? "FFFFFF" : route_pt.route_text_color);
    ...
    routeMapping.insert(make_pair(route_pt.route_id,std::make_pair(route->onestop_id(), routeMapping.size())));
  }
}

void save_stop_pair(Transit& tile, 
                const gtfs::Feed& feed, 
                const AABB2<PointLL>& bbox, 
                const GraphId& tile_id,
  ...
  for(auto& trip: trips) {
    ...
    if(!calender) {
      LOG_ERROR("Please check your GTFS feed, calender corresponding to service ID" + trip.service_id + " not found" );
      return;
    }
    ...
    for(auto i = 0; i < stop_times.size() -1; i++) {
      ...
      if(!destination) {
        LOG_ERROR("Please check your GTFS feed, stop with id" + destination.get().stop_id + " not found" );
        return;
      }
      ...
      if (origin_stop_mapping != platformMapping.cend()) {
        ...
        LOG_ERROR("Cannot happen please file an issue If found this log, origin_stop_mapping not found" );
        return;
      }
      ...
      if (destination_stop_mapping != platformMapping.cend()) {
        ...
        LOG_ERROR("Cannot happen please file an issue If found this log, destination_stop_mapping not found" );
        return;
      }
      ...
      for(const auto& calender_date : calender_dates) {
        if(calender_date.exception_type == gtfs::CalendarDateException::Added) {
          pair->add_service_added_dates(DateTime::days_from_pivot_date(DateTime::get_formatted_date(calender_date.date.get_raw_date())));
        } else {
          pair->add_service_except_dates(DateTime::days_from_pivot_date(DateTime::get_formatted_date(calender_date.date.get_raw_date())));
        }
      }
...
void build_tiles(const ptree& pt, 
                 std::queue<GraphId>& queue,
                 const gtfs::Feed& feed) {
