  ...
  for (auto& nb : currentnodes) {
    ...
    while (added_edges < connection_edges.size() &&
      ...
      uint32_t edge_info_offset =
          tilebuilder_local.AddEdgeInfo(0, conn.osm_node, endnode, conn.wayid, 0, 0, 0, false,
  ...
  for (auto& nb : currentnodes) {
    ...
    for (const auto& conn : connection_edges) {
      if (conn.stop_node.id() == nb.stop_index()) {
        ...
        uint32_t edge_info_offset =
            tilebuilder_transit.AddEdgeInfo(0, origin_node, conn.osm_node, conn.wayid, 0, 0, 0, false,
