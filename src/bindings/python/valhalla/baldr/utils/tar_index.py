...
# One entry per tile, packed little-endian without padding: the uint64 must
# come first or C++ would read the struct with padding bytes.
...
def decode_tar_index(data: bytes) -> list[TarIndexEntry]:
    """Decode a raw index.bin body (e.g. fetched with an HTTP range request).

    :param data: The index.bin bytes.
    :returns: One entry per tile, in file order.
    :raises ValueError: ``data`` isn't a whole number of entries.
    """
...
def read_tar_index(tar_path: Union[str, Path]) -> list[TarIndexEntry]:
    """Read the embedded tile index from a tar extract.

    :param tar_path: Path of the (uncompressed) tile tar.
    :returns: One entry per tile, in file order.
    :raises ValueError: The tar's first member isn't ``index.bin``.
    """
    with tarfile.open(tar_path, "r") as tar:
        ...
        return decode_tar_index(tar.extractfile(first).read())
