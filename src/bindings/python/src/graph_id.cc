...
void init_graphid(nb::module_& m) {
  ...
  // Standalone get_edge_shape function for convenience
  m.def(
      "get_edge_shape",
      [](const vb::GraphId& edge_id, const std::string& config) {
        // Create temporary GraphReader
        boost::property_tree::ptree pt;
        try {
          std::ifstream file(config);
          if (file.good()) {
