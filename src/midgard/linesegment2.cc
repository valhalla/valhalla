...
namespace midgard {
  ...
  typename coord_t::value_type LineSegment2<coord_t>::DistanceSquared(const coord_t& p, coord_t& closest) const {
   ...
   */
  template <typename coord_t>
  bool LineSegment2<coord_t>::Intersect(const LineSegment2<coord_t>& segment, coord_t& intersect) const {
    using prec_t = typename coord_t::value_type;
    // Construct vectors
   ...
   */
  template <typename coord_t>
  bool LineSegment2<coord_t>::ClipToPolygon(const std::vector<coord_t>& poly, LineSegment2<coord_t>& clip_segment) const {
    using prec_t = typename coord_t::value_type;
    // Initialize the candidate interval
    prec_t t_out = 1;
    prec_t t_in = 0;
