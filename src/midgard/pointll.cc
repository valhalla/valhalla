...
namespace valhalla{
  ...
  GeoPoint<PrecisionT> GeoPoint<PrecisionT>::MidPoint(const GeoPoint<PrecisionT>& p) const {
    ...
    return GeoPoint<PrecisionT>(atan2(y, x) * -DEG_PER_RAD, atan2(z, sqrt(x * x + y * y)) * DEG_PER_RAD);
  }
   ...
   */
  template<typename PrecisionT>
  PrecisionT GeoPoint<PrecisionT>::Distance(const GeoPoint& ll2) const {
   ...
   */
  template<typename PrecisionT>
  PrecisionT GeoPoint<PrecisionT>::Heading(const GeoPoint& ll2) const {
    // If points are the same, return 0
    if (*this == ll2) {
      return 0.0;
    }
   ...
   */
  template <typename PrecisionT>
  template <typename container_t> bool GeoPoint<PrecisionT>::WithinPolygon(const container_t& poly) const {
   ...
   */
  template<typename PrecisionT>
  GeoPoint<PrecisionT> GeoPoint<PrecisionT>::Project(const GeoPoint& u, const GeoPoint& v, PrecisionT lon_scale) const {
   ...
   */
  template<typename PrecisionT>
  std::tuple<GeoPoint<PrecisionT>, PrecisionT, int> GeoPoint<PrecisionT>::Project(const std::vector<GeoPoint>& pts) const {
    auto u = pts.begin();
    auto v = pts.begin();
    std::advance(v, 1);
