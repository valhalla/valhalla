...
LaneConnectivity::LaneConnectivity(const uint32_t idx, const uint64_t from,
  const std::string& to_lanes, const std::string& from_lanes)
    : to_(idx), from_(from),
    to_lanes_(to_lanes), from_lanes_(from_lanes)
{
  if (from >= (1LL << 42)) {
...
// Constructor with arguments.
LaneConnectivityLanes::LaneConnectivityLanes(const std::string& lanes)
  : value_(0)
{
