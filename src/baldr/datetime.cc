...
std::string conditional_to_str(const bool type,
  ...
  if (type == kYMD) { // kYMD - use days of the month
    if (begin_month != 0) {
    ...
    if (end_month != 0 && (end_month != begin_month || end_day_dow != begin_day_dow)) {
      if (end_month != begin_month || end_day_dow != 0) {
        ss << '-';
      }
      if (end_month != 0) {
  ...
  // Handle time range
  if (begin_hrs != 0 || end_hrs != 0 || begin_mins != 0 || end_mins != 0) {
    if (need_space) {
      ss << ' ';
    }
    ss << std::setw(2) << std::setfill('0') << static_cast<int>(begin_hrs) << ':' << std::setw(2)
       << std::setfill('0') << static_cast<int>(begin_mins);
    if (end_hrs != 0 || end_mins != 0) {
      ss << '-' << std::setw(2) << std::setfill('0') << static_cast<int>(end_hrs) << ':'
         << std::setw(2) << std::setfill('0') << static_cast<int>(end_mins);
    }
  }
