...
inline uint16_t get_outbound_edge_heading(const boost::intrusive_ptr<const baldr::GraphTile>& tile,
...
find_shortest_path(baldr::GraphReader& reader,
  ...
  expand = [&](const baldr::GraphId& node, const uint32_t label_idx, const bool from_transition) {
    ...
    boost::intrusive_ptr<const baldr::GraphTile> tile = reader.GetGraphTile(node);
    if (tile == nullptr) {
      return;
    }
    ...
    for (uint32_t i = 0; i < nodeinfo->edge_count(); ++i, ++directededge, ++edgeid) {
      ...
      boost::intrusive_ptr<const baldr::GraphTile> endtile =
  ...
  while (true) {
    ...
    if (label.nodeid().Is_Valid()) {
      ...
      for (const auto& origin_edge : origin.edges) {
        ...
        boost::intrusive_ptr<const baldr::GraphTile> start_tile = nullptr;
