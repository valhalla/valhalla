...
inline bool IsEdgeAllowed(const baldr::DirectedEdge* edge,
                          ...
                          const graph_tile_ptr& tile,
                          int& restriction_idx) {
                            bool valid_pred = (!pred_edgelabel.edgeid().Is_Valid() &&
                                               costing->Filter(edge, tile) != 0.f) ||
                                              edgeid == pred_edgelabel.edgeid();
                            bool restricted = !costing->Allowed(edge, pred_edgelabel, tile, edgeid, 0,
                                                                0, restriction_idx);
                          ...
                          void set_origin(baldr::GraphReader & reader,
                                          ...
                                          uint16_t origin_idx, const labelset_ptr_t& labelset,
                                          const sif::TravelMode travelmode,
                                          const sif::cost_ptr_t& costing, const Label* edgelabel) {
                          ...
                          void set_destinations(baldr::GraphReader & reader,
                                                ...
                                                std::unordered_map<baldr::GraphId,
                           ...
                           */
                          inline uint16_t
                          get_inbound_edgelabel_heading(baldr::GraphReader & reader,
                                                        const Label& label,
                          ...
                          inline uint16_t
                          get_outbound_edge_heading(const graph_tile_ptr& tile,
                          ...
                          find_shortest_path(baldr::GraphReader & reader,
                                             ...
                                             const midgard::DistanceApproximator<midgard::PointLL>&
                                                 approximator,
                            ...
                            std::unordered_map<baldr::GraphId, std::unordered_set<uint16_t>>
                                edge_dests;
                            ...
                            std::function<void(const baldr::GraphId&, const uint32_t, const bool)>
                                expand;
                            expand = [&](const baldr::GraphId& node, const uint32_t label_idx,
                              ...
                              const auto inbound_hdg =
                                  label.edgeid().Is_Valid()
                                      ? get_inbound_edgelabel_heading(reader, label, nodeinfo)
                                      : 0;

                              // Expand from end node in forward direction.
                              baldr::GraphId edgeid = {node.tileid(), node.level(),
                                                       nodeinfo->edge_index()};
                              ...
                              for (uint32_t i = 0; i < nodeinfo->edge_count();
                                ...
                                if (label.edgeid().Is_Valid()) {
                                  const auto outbound_hdg =
                                      get_outbound_edge_heading(tile, directededge, nodeinfo);
                                  turn_cost +=
                                      turn_cost_table[midgard::get_turn_degree180(inbound_hdg,
                                                                                  outbound_hdg)];
                                }
                                    ...
                                    for (const auto& edge : destinations[dest].edges) {
                                      if (edge.id == edgeid) {
                                        ...
                                        sif::Cost cost(label.cost().cost + directededge->length() *
                                                                               edge.percent_along,
                                                       label.cost().secs +
                                                           costing->EdgeCost(directededge, tile)
                                                                   .secs *
                                                               edge.percent_along);
                                        // We only add the labels if we are under the limits for
                                        // distance and for time or time limit is 0
                                        if (cost.cost < max_dist &&
                                            (max_time < 0 || cost.secs < max_time)) {
                                          labelset->put(dest, edgeid, 0.f, edge.percent_along, cost,
                                                        turn_cost, cost.cost, label_idx, directededge,
                                                        travelmode, restriction_idx);
                                        }
                                      }
                                    }
                                  }
                                ...
                                if (endtile != nullptr) {
                                  ...
                                  sif::Cost cost(label.cost().cost + directededge->length(),
                                                 label.cost().secs +
                                                     costing->EdgeCost(directededge, tile).secs);
                                  ...
                                  if (cost.cost < max_dist &&
                                    ...
                                    float sortcost =
                                        cost.cost +
                                        heuristic(endtile->get_node_ll(directededge->endnode()));
                                    labelset->put(directededge->endnode(), edgeid, 0.0f, 1.0f, cost,
                                                  turn_cost, sortcost, label_idx, directededge,
                                                  travelmode, restriction_idx);
                                  }
                                }
                              }
                              ...
                              if (!from_transition && nodeinfo->transition_count() > 0) {
                                const baldr::NodeTransition* trans =
                                    tile->transition(nodeinfo->transition_index());
                            ...
                            set_origin(reader, destinations, origin_idx, labelset, travelmode,
                                       costing, edgelabel);
                              ...
                              if (label.nodeid().Is_Valid()) {
                                ...
                                for (const auto& origin_edge : origin.edges) {
                                  ...
                                  const auto* directed_edge =
                                      reader.directededge(origin_edge.id, start_tile);

                                  // Skip if edge is not allowed
                                  uint8_t restriction_idx = -1;
                                  if (!directed_edge ||
                                      !IsEdgeAllowed(directed_edge, origin_edge.id, costing, label,
                                  ...
                                  for (const auto other_dest : edge_dests[origin_edge.id]) {
                                    ...
                                    for (const auto& destination_edge :
                                         destinations[other_dest].edges) {
                                      if (origin_edge.id == destination_edge.id &&
                                        ...
                                        float segment_percentage = (destination_edge.percent_along -
                                                                    origin_edge.percent_along);
                                        sif::Cost cost(label.cost().cost + directed_edge->length() *
                                                                               segment_percentage,
                                                       label.cost().secs +
                                                           costing->EdgeCost(directed_edge,
                                                                             start_tile)
                                                                   .secs *
                                                               segment_percentage);
                                        // We only add the labels if we are under the limits for
                                        ...
                                        if (cost.cost < max_dist &&
                                          ...
                                          labelset->put(other_dest, origin_edge.id,
                                                        origin_edge.percent_along,
                                                        destination_edge.percent_along, cost,
                                                        turn_cost, cost.cost, label_idx,
                                  ...
                                  sif::Cost
                                      cost(label.cost().cost + directed_edge->length() * f,
                                           label.cost().secs +
                                               costing->EdgeCost(directed_edge, start_tile).secs * f);
                                  // We only add the labels if we are under the limits for distance
                                  // and for time or time limit is 0
                                  if (cost.cost < max_dist &&
                                      (max_time < 0 || cost.secs < max_time)) {
                                    ...
                                    float sortcost =
                                        cost.cost +
                                        heuristic(endtile->get_node_ll(directed_edge->endnode()));
                                    labelset->put(directed_edge->endnode(), origin_edge.id,
                                                  origin_edge.percent_along, 1.f, cost, turn_cost,
                                                  sortcost, label_idx, directed_edge, travelmode,
                                                  restriction_idx);
                                  }
                                }
                              }
