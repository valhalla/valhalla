...
sample::tile_data::~tile_data() {
  if (reusable) {
    s->decrement_usages(index);
  }
}
...
void sample::increment_usages(uint16_t index) {
...
template <class coords_t> std::vector<double> sample::get_all(const coords_t& coords) {
  ...
  tile_data tile(this, TILE_COUNT, false, nullptr);
