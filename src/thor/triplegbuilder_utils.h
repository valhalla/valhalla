...
struct MultimodalBuilder {
  ...
  void Build(valhalla::TripLeg::Node* trip_node,
             ...
             const GraphId& edge,
             boost::intrusive_ptr<const GraphTile> start_tile,
             boost::intrusive_ptr<const GraphTile> graphtile,
      ...
      if (transit_platform) {
        ...
        for (uint32_t index = 0; index < node->edge_count(); ++index, dir_edge++) {
          if (dir_edge->use() == Use::kPlatformConnection) {
            ...
            boost::intrusive_ptr<const GraphTile> endtile = graphreader.GetGraphTile(endnode);
