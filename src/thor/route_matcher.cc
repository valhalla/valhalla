...
bool RouteMatcher::FormPath(
  ...
  const PathLocation::PathEdge* begin_path_edge = FindBeginEdge(correlated);
  ...
  const PathLocation::PathEdge* end_path_edge = FindEndEdge(correlated);
  ...
  const GraphId end_edge_start_node = FindStartNode(reader, end_path_edge->id);
  ...
  while (index < shape.size()
    ...
    if (shape.at(index).ApproximatelyEqual(de_end_ll)) {
      ...
      if (RouteMatcher::ExpandFromNode(mode_costing, mode, reader, shape, index,
...
const PathLocation::PathEdge* RouteMatcher::FindBeginEdge(
    const std::vector<baldr::PathLocation>& correlated) {
...
const PathLocation::PathEdge* RouteMatcher::FindEndEdge(
    const std::vector<baldr::PathLocation>& correlated) {
...
const GraphId RouteMatcher::FindStartNode(GraphReader& reader,
...
bool RouteMatcher::ExpandFromNode(
    const std::shared_ptr<sif::DynamicCost>* mode_costing,
  ...
  for (uint32_t i = 0; i < node_info->edge_count(); i++, de++, edge_id++) {
    ...
    if (de->trans_down() || de->trans_up()) {
      if (from_transition) {
        ...
        if (ExpandFromNode(mode_costing, mode, reader, shape, correlated_index,
                           end_node_tile, de->endnode(), stop_node,
                           prev_edge_label, elapsed_time, path_infos, true)) {
          return true;
        } else {
          continue;
        }
    ...
    while (index < shape.size()
      ...
      if (shape.at(index).ApproximatelyEqual(de_end_ll)) {
        ...
        // Continue walking shape to find the end edge...
        return (ExpandFromNode(mode_costing, mode, reader, shape, index,
                               end_node_tile, de->endnode(), stop_node,
