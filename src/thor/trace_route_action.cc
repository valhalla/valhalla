...
thor_worker_t::map_match(Api& request) {
  ...
  for (const auto& result : topk_match_results) {
    ...
    auto paths = MapMatcher::FormPath(matcher.get(), match_results, edge_segments, mode_costing, mode,
                                      disconnected_edges, options);
    ...
    if (options.action() == Options::trace_attributes &&
      ...
      //              auto& edge_end_info =
      //              route_discontinuities.at(prev_match_result->edge_index).second;
      ...
      //              route_discontinuities.at(curr_match_result->edge_index).first;
      ...
      //               marker_size.c_str(), index++, matched_point_type.c_str(),
      //               match_result.edge_index, match_result.distance_along,
      //               match_result.distance_from,
    ...
    if (options.action() == Options::trace_attributes) {
      ...
      serilize_paths_new(paths, match_results, route_discontinuities, options, request);
    }
...
void thor_worker_t::serilize_paths_new(
...
void thor_worker_t::serilize_paths(
    const std::vector<PathInfo>& path_edges,
    const std::vector<meili::MatchResult>& match_results,
    const std::vector<std::pair<GraphId, GraphId>>& disconnected_edges,
    std::unordered_map<size_t, std::pair<RouteDiscontinuity, RouteDiscontinuity>>&
        route_discontinuities,
    Options& options,
    Api& request) {
  ...
  for (auto& edge_group : edge_groups) {
    ...
    match_result_itr =
        std::find_if(match_result_itr, match_results.cend(), [first_edge](const MatchResult& result) {
    ...
    for (auto path_edge_itr = std::get<0>(edge_group);
      ...
      for (auto destination_match_result = std::next(origin_match_result);
        ...
        if (origin_match_result->edgeid == destination_match_result->edgeid &&
          ...
          path_edge_itr = std::find_if(std::next(path_edge_itr), std::next(std::get<1>(edge_group)),
        ...
        std::cout << "trim_begin before " << trim_begin << std::endl;
        ...
        std::cout << leg_begin->elapsed_time << " " << origin_match_result->distance_along << " "
                  << begin_pct << " " << begin_edge_scale << std::endl;
        std::cout << "trim_begin after " << trim_begin << std::endl;
        ...
        std::cout << "trim_end before " << trim_end << std::endl;

        // then we scale the elapsed time on the edge based on the distance along the edge but
        // we need to scale that distance down in the case FormPath knew it was a partial edge
        trim_end = (leg_end->elapsed_time - trim_end) *
                   (end_pct - destination_match_result->distance_along) * end_edge_scale;
        std::cout << "trim_end after " << trim_end << std::endl;

        // add a new leg to the current route
        std::cout << " trim begin " << trim_begin << ", trim end " << trim_end << std::endl;
        std::cout << "build legs on " << std::distance(match_results.cbegin(), origin_match_result)
                  << " " << std::distance(match_results.cbegin(), destination_match_result)
                  << std::endl;
