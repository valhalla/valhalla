...
#include "thor/costmatrix_astar.h"
#include "worker.h"
...
class CostMatrixAstar::ReachedMap
    : public robin_hood::unordered_map<uint64_t, std::vector<uint32_t>> {};

// Constructor with cost threshold.
CostMatrixAstar::CostMatrixAstar(const boost::property_tree::ptree& config)
...
CostMatrixAstar::~CostMatrixAstar() {
}

float CostMatrixAstar::GetCostThreshold(const float max_matrix_distance) {
...
bool CostMatrixAstar::SourceToTarget(Api& request,
                                     baldr::GraphReader& graphreader,
                                     const sif::mode_costing_t& mode_costing,
                                     const sif::travel_mode_t mode,
                                     const float max_matrix_distance) {
  request.mutable_matrix()->set_algorithm(Matrix::CostMatrixAStar);
...
void CostMatrixAstar::Initialize(
...
bool CostMatrixAstar::ExpandInner(baldr::GraphReader& graphreader,
...
template <const MatrixExpansionType expansion_direction, const bool FORWARD>
bool CostMatrixAstar::Expand(const uint32_t index,
                             const uint32_t n,
                             baldr::GraphReader& graphreader,
                             const baldr::TimeInfo& time_info,
...
void CostMatrixAstar::CheckForwardConnections(const uint32_t source,
...
void CostMatrixAstar::CheckReverseConnections(const uint32_t target,
...
void CostMatrixAstar::UpdateStatus(const uint32_t source, const uint32_t target) {
...
// locations.
void CostMatrixAstar::SetSources(
    GraphReader& graphreader,
...
// these locations.
void CostMatrixAstar::SetTargets(
    baldr::GraphReader& graphreader,
...
std::string CostMatrixAstar::RecostFormPath(GraphReader& graphreader,
  ...
  for (auto edgelabel_index = connedge_idx1; edgelabel_index != kInvalidLabel;
    ...
    if (edge == nullptr) {
      throw tile_gone_error_t("CostMatrixAstar::RecostPaths failed", edgelabel.edgeid());
    }
  ...
  if (has_time_) {
    ...
    try {
      ...
      LOG_ERROR(std::string("CostMatrixAstar failed to recost final paths: ") + e.what());
      return "";
    }
