  ...
  for (auto source : sources->second) {
    ...
    if (fwd_edgestatus.set() != EdgeSet::kUnreachedOrReset) {
      ...
      if (rev_pred.predecessor() == kInvalidLabel && fwd_predidx == kInvalidLabel) {
        ...
        auto& source_edge = find_correlated_edge(options.sources(source), fwd_label.edgeid());
        auto& target_edge = find_correlated_edge(options.targets(target), fwd_label.edgeid());
