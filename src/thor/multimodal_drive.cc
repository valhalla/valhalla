...
static travel_mode_t get_other_travel_mode(const travel_mode_t current_mode) {
  static const auto possible_modes =
      std::vector<travel_mode_t>{travel_mode_t::kPedestrian, travel_mode_t::kDrive};
  return possible_modes[static_cast<size_t>(current_mode == travel_mode_t::kPedestrian)];
}
} // namespace
...
// Default constructor
MultimodalDrive::MultimodalDrive(const boost::property_tree::ptree& config)
...
// Destructor
MultimodalDrive::~MultimodalDrive() {
}

// Clear the temporary information generated during path construction.
void MultimodalDrive::Clear() {
...
void MultimodalDrive::Init(const midgard::PointLL& origll, const midgard::PointLL& destll) {
  ...
  uint32_t bucketsize = std::max(pedestrian_costing_->UnitSize(), drive_costing_->UnitSize());
...
void MultimodalDrive::ExpandForward(GraphReader& graphreader,
                                    const GraphId& node,
                                    const BDEdgeLabel& pred,
                                    const uint32_t pred_idx,
                                    const bool from_transition,
                                    const bool from_parking,
                                    const sif::travel_mode_t mode,
                                    const valhalla::Location& destination,
                                    std::pair<int32_t, float>& best_path) {
  const auto& current_costing =
      (mode == travel_mode_t::kPedestrian ? pedestrian_costing_ : drive_costing_);
  const auto& current_heuristic =
      (mode == travel_mode_t::kPedestrian ? pedestrian_astarheuristic_ : drive_astarheuristic_);
  ...
  EdgeStatus& current_es =
      (mode == travel_mode_t::kPedestrian ? pedestrian_edgestatus_ : drive_edgestatus_);
  ...
  for (uint32_t i = 0; i < nodeinfo->edge_count(); ++i, ++meta) {
    ...
    auto p =
        mode == travel_mode_t::kPedestrian ? destinations_.find(meta.edge_id) : destinations_.end();
    ...
    edgelabels_.emplace_back(pred_idx, meta.edge_id, GraphId(), meta.edge, newcost, sortcost, dist,
                             mode, transition_cost, false, false, false, InternalTurn::kNoTurn,
                             baldr::kInvalidRestriction);
    *meta.edge_status = {EdgeSet::kTemporary, idx};
    adjacencylist_.add(idx);

    if (expansion_callback_) {
      expansion_callback_(graphreader, meta.edge_id, GraphId(pred.edgeid()), "multimodal_drive",
                          ...
                          mode == sif::TravelMode::kPedestrian ? TravelMode::kPedestrian
                                                               : TravelMode::kDrive);
    }
  }
...
MultimodalDrive::GetBestPath(valhalla::Location& origin,
  ...
  while (true) {
    ...
    if (!pred.origin() && pred.mode() == travel_mode_t::kDrive) {
      drive_edgestatus_.Update(pred.edgeid(), EdgeSet::kPermanent);
    }
    if (expansion_callback_) {
      expansion_callback_(graphreader, pred.edgeid(),
                          GraphId(pred.predecessor() == kInvalidLabel
                                      ? kInvalidGraphId
                                      : edgelabels_[pred.predecessor()].edgeid()),
                          "multimodal_drive", Expansion_EdgeStatus_settled, pred.cost().secs,
                          pred.distance(), pred.cost().cost,
                          Expansion_ExpansionType::Expansion_ExpansionType_forward, kNoFlowMask,
                          pred.mode() == sif::TravelMode::kPedestrian ? TravelMode::kPedestrian
                                                                      : TravelMode::kDrive);
...
void MultimodalDrive::SetOrigin(GraphReader& graphreader,
  ...
  for (const auto& edge : origin.correlation().edges()) {
    ...
    Cost cost =
        drive_costing_->PartialEdgeCost(directededge, edgeid, tile, edge.percent_along(), 1.0f);
    float dist = drive_astarheuristic_.GetDistance(endtile->get_node_ll(directededge->endnode()));
    ...
    BDEdgeLabel edge_label(kInvalidLabel, edgeid, directededge, cost, sortcost, dist,
                           travel_mode_t::kDrive, baldr::kInvalidRestriction, false, false,
    ...
    if (expansion_callback_) {
      expansion_callback_(graphreader, edgeid, GraphId(kInvalidGraphId), "multimodal_drive",
...
void MultimodalDrive::SetDestination(GraphReader& graphreader, const valhalla::Location& dest) {
...
std::vector<PathInfo> MultimodalDrive::FormPath(baldr::GraphReader& graphreader,
                                                const uint32_t dest) {
  // Metrics to track
