...
edge_t opposing(GraphReader& reader, graph_tile_ptr tile, const DirectedEdge* edge) {
  graph_tile_ptr t = edge->leaves_tile() ? reader.GetGraphTile(edge->endnode()) : std::move(tile);
