...
constexpr float kDefaultSideWalkFactor = 0.25f;    // Slightly favor sidewalks
...
Cost PedestrianCost::EdgeCost(const baldr::DirectedEdge* edge, const uint32_t speed) const {
  ...
  if (edge->use() == Use::kFootway or edge->use() == Use::kSidewalk) {
  ...
  } else if (edge->sidewalk_left() or edge->sidewalk_right()) {
