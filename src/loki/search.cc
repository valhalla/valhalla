...
/*
  //a place to keep information about the edges we've already visited
  //we must do this because each edge can have an opposing edge and we
  //really dont want to decode the shape twice for the same edge
  std::unordered

  //a place to keep the closest information so far
  const DirectedEdge* edge = tile->directededge(0);
  auto closest = location.latlng_.ClosestPoint(tile->edgeinfo(edge->edgedataoffset())->shape());

  //for each edge
  for(size_t edge_index = 1; edge_index < tile->header()->nodecount(); ++edge_index) {
    //if this is closer then its better
    const NodeInfo* node = tile->node(node_index);
    float node_sqdist = node->latlng().DistanceSquared(location.latlng_);
    if(node_sqdist < sqdist) {
