...
make_connectivity_map(const boost::property_tree::ptree& config,
  ...
  if (reader->tile_extract().empty() && !config.get<std::string>("mjolnir.tile_url", "").empty()) {
    LOG_INFO(
        "Connectivity map disabled: tile source is lazy loaded via HTTP, i.e. mjolnir.tile_url is set");
    return nullptr;
  }
...
loki_worker_t::loki_worker_t(const boost::property_tree::ptree& config,
    ...
    : service_worker_t(config), config(config),
      ...
      search_(*reader), connectivity_map(make_connectivity_map(config, reader)),
