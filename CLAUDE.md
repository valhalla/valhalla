...
| Tile checksum / build id (cache validation) | `GraphTileHeader::checksum_` packs `build_id<<48 \| per_tile_data_hash` (`kTileHashBits` in `valhalla/baldr/graphconstants.h`); `header->checksum()` returns the low-48 data hash, `header->build_id()` the high-16 tileset id a `tile_url` client compares against `id.txt`. Primitives in `src/mjolnir/util.cc`: `set_tile_checksum(path)` (refresh one tile's hash, keep build id) and `set_tileset_build_id(tile_dir)` (recompute the build id from stored hashes). **Any new tool that rewrites existing tiles MUST re-hash:** call `set_tile_checksum` on each tile after writing it, then `set_tileset_build_id` once at the end. Placement rule: if the function is also a build stage (e.g. `ElevationBuilder::Build`), put the calls in the **exe** (the build's validate stage already checksums); if it's standalone-only (e.g. `ProcessTrafficTiles`, `AddLandmarks`), put them in the **library** entry point. See `build_tile_set` (validate stage), `add_predicted_speeds.cc`, `landmarks.cc`, `valhalla_add_elevation.cc`, `valhalla_assign_speeds.cc` |
