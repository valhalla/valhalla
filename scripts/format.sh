...
# Python formatter
${py} -m black --config src/bindings/python/pyproject.toml --line-length=105 --skip-string-normalization ${python_sources}

# Python linter
${py} -m flake8 --config src/bindings/python/setup.cfg \
  --max-line-length=105 \
  --extend-ignore=E501,E731,E203 \
  --extend-exclude=src/bindings/python/valhalla/__init__.py \
  ${python_sources}
