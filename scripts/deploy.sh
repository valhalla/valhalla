...
if [ $WITH_UPDATES ]; then
  ...
  (crontab -l 2>/dev/null; echo "*/5 * * * * cd ${base_dir}; WITH_UPDATES=${WITH_UPDATES} ${src_dir}/mjolnir/scripts/update_tiles.sh ${base_dir} ${config} ${src_dir} ${extracts_dir} >> ${log_dir}/update_cron.log 2>&1") | crontab -
