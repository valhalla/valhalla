...
valhalla::gurka::map BuildPBF(const std::string& workdir) {
  ...
  const gurka::nodes nodes = {{"1", {{"highway", "give_way"}, {"direction", "backward"}}},
                              {"2", {{"highway", "give_way"}, {"direction", "forward"}}},
                              {"3", {{"highway", "give_way"}}},
                              {"B", {{"highway", "give_way"}, {"give_way", "minor"}}},
                              {"O", {{"highway", "give_way"}}},
                              {"4", {{"highway", "give_way"}, {"direction", "forward"}}},
                              {"5", {{"highway", "give_way"}}}};
...
TEST(Standalone, yieldSigns) {
  ...
  // edges should not be marked with a yield sign as they are not minor
  {
    GraphId AB_edge_id;
    ...
    EXPECT_EQ(AB_edge->yield_sign(), false);
    EXPECT_EQ(BA_edge->yield_sign(), false);
  }

  // CB edge should be marked with a yield sign as there is a yield sign in the backward direction
  {
    GraphId BC_edge_id;
    ...
    EXPECT_EQ(BC_edge->yield_sign(), false);
    EXPECT_EQ(CB_edge->yield_sign(), true);
  }

  // FB should be marked with a yield sign
  {
    GraphId BF_edge_id;
    ...
    EXPECT_EQ(BF_edge->yield_sign(), false);
    EXPECT_EQ(FB_edge->yield_sign(), true);
  }

  // BI should be marked with a yield sign
  {
    GraphId BI_edge_id;
    ...
    EXPECT_EQ(BI_edge->yield_sign(), false);
    EXPECT_EQ(IB_edge->yield_sign(), true);
  }

  // edge DC should only be marked with a yield sign
  {
    GraphId DC_edge_id;
    ...
    EXPECT_EQ(DC_edge->yield_sign(), true);
    EXPECT_EQ(CD_edge->yield_sign(), false);
  }

  // edges CE should be marked with a yield sign as there is no direction.
  // due to the onewayness on EC, we toss the yield sign
  {
    GraphId CE_edge_id;
    ...
    EXPECT_EQ(CE_edge->yield_sign(), true);
    EXPECT_EQ(EC_edge->yield_sign(), false);
  }

  {
    ...
    EXPECT_EQ(NO_edge->yield_sign(), true);
    EXPECT_EQ(ON_edge->yield_sign(), false);
    ...
    EXPECT_EQ(OM_edge->yield_sign(), false);
    EXPECT_EQ(MO_edge->yield_sign(), true);
    ...
    EXPECT_EQ(OQ_edge->yield_sign(), false);
    EXPECT_EQ(QO_edge->yield_sign(), true);
    ...
    EXPECT_EQ(TO_edge->yield_sign(), false); // false here due to being a oneway edge
    EXPECT_EQ(OT_edge->yield_sign(), false);
  }

  {
    ...
    EXPECT_EQ(RM_edge->yield_sign(), true);
    EXPECT_EQ(MR_edge->yield_sign(), false);
  }

  {
    ...
    EXPECT_EQ(TU_edge->yield_sign(), true);
    EXPECT_EQ(UT_edge->yield_sign(), false); // no yield sign as we are a oneway road
  }
}
