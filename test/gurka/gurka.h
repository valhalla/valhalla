...
using HistoricalTrafficCustomize =
    std::function<boost::optional<std::array<float, baldr::kBucketsPerWeek>>(baldr::DirectedEdge&)>;
void customize_historical_traffic(const boost::property_tree::ptree& config,
                                  const HistoricalTrafficCustomize& cb);

using EdgesCustomize = std::function<void(const baldr::GraphId&, baldr::DirectedEdge&)>;
