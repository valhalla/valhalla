...
//         // all restrictions should be higher than our defaults, so we can actually see the impact
//         of
//         // any single one
//         {"BC",
//          {{"highway", "residential"},
...
//                                            std::pair<std::string, std::string>{"axle_count",
//                                            "10"}));
...
// make sure forward/backward restrictions are considered
TEST(Standalone, DirectionalRestrictions) {
  constexpr double gridsize = 500;
  ...
  const gurka::ways ways = {
      {"AB", {{"highway", "residential"}}},
      {"BC", {{"highway", "residential"}, {"maxheight:forward", "5.0"}}},
      {"CD", {{"highway", "residential"}}},
  };
  ...
  // int access_restrictions = 0;
  // for (const GraphId& tile_id : tiles) {

  //   graph_tile_ptr tile = reader.GetGraphTile(tile_id);
  //   const GraphTileHeader* header = tile->header();
  //   access_restrictions += header->access_restriction_count();
  //   std::cout << "Found " << header->access_restriction_count() << " ars on level " <<
  //   tile_id.level()
  //             << "\n";
  //   uint32_t edges = tile->header()->directededgecount();
  //   for (uint32_t i = 0; i < edges; i++) {
  //     auto edge_id = GraphId(tile->id().tileid(), tile->id().level(), i);
  //     auto ars = tile->GetAccessRestrictions(edge_id.id(), baldr::kTruckAccess);
  //     auto* edge = tile->directededge(edge_id);
  //     const auto& name = reader.edgeinfo(edge_id);
  //     std::cout << name.GetNames()[name.name_count() - 1] << ", " << (edge->forward() ? "fwd" :
  //     "rev")
  //               << ": " << static_cast<uint64_t>(edge_id) << " | " << ars.size() << "\n";
  //   }
  // }

  try {
    valhalla::Api default_route = gurka::do_action(valhalla::Options::route, map, {"A", "D"}, "truck",
                                                   {
