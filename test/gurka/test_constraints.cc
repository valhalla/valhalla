...
{
  sif::CostConstraints::StaticComputeConstraintForEdgeF =  Constraints::GetConstraint;
  EXPECT_THROW(gurka::route(map, std::vector<std::string>{"A", "D"}, "auto", "{\"auto\":{ \"constraints\":[{\"max_inclusive\":true,\"min_inclusive\":true,\"max_value\":3.0}]}}"), std::exception);
}