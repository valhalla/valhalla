...
TEST(MatrixTest, MatrixSimple) {
  ...
  const auto map = gurka::buildtiles(layout, ways, {}, {}, "test/data/matrix");
...
TEST(MatrixTest, MatrixLocations) {
  constexpr double gridsize = 1000;
  ...
  const auto layout = gurka::detail::map_to_coordinates(ascii_map, gridsize);
  const auto map = gurka::buildtiles(layout, ways, {}, {}, "test/data/matrix_locations");

  const std::vector<double> exp_dists = {3.f, 8.f, 8.f, 3.f};
  const std::vector<uint32_t> exp_times = {308, 823, 823, 308};
  std::string res;
  const auto result =
      gurka::do_action(Options::sources_to_targets, map, {"A", "B", "C", "D"}, {"A", "B", "C", "D"},
                       "auto", {{"/matrix_locations", "0"}}, {}, &res);
  ...
  for (const auto& origin_row : res_doc["sources_to_targets"].GetArray()) {
    ...
    for (const auto& v : origin_td) {
      std::string msg = "Problem at source " + std::to_string(i / origin_td.Size()) + " and target " +
                        std::to_string(i % origin_td.Size());
      std::cerr << std::to_string(v.GetObject()["distance"].GetDouble()) << std::endl;
      if (v.GetObject()["distance"].GetDouble() > 0.0) {
        found++;
      };
      i++;
    }
