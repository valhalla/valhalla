...
TEST_F(TaggedValues, Tunnel) {
  ...
  EXPECT_EQ(leg.node(0).edge().has_tunnel_case(), false);
  ...
  EXPECT_EQ(leg.node(4).edge().has_tunnel_case(), false);

  EXPECT_EQ(leg.node(5).edge().has_tunnel_case(), false);
}
...
TEST_F(TaggedValues, NoTunnel) {
  ...
  auto leg = result.trip().routes(0).legs(0);
  EXPECT_EQ(leg.node(0).edge().has_tunnel_case(), false);
  EXPECT_EQ(leg.node(1).edge().has_tunnel_case(), false);
  EXPECT_EQ(leg.node(2).edge().has_tunnel_case(), false);
}
