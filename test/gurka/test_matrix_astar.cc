...
TEST_F(MatrixTrafficTest, MatrixNoTraffic) {
  ...
  const auto result = gurka::do_action(Options::sources_to_targets, map, {"E", "L"}, {"E", "L"},
                                       "auto", {{"/use_costmatrix_astar", "1"}}, {}, &res);

  rapidjson::Document res_doc;
  res_doc.Parse(res.c_str());

  // we expect to take the motorways, residential path is 2.8f
  check_matrix(res_doc, {0.0f, 3.2f, 3.2f, 0.0f}, false, Matrix::CostMatrixAStar);
}
...
TEST_F(MatrixTrafficTest, TDMatrixWithLiveTraffic) {
  ...
  result = gurka::do_action(Options::sources_to_targets, map, {"E", "L"}, {"L"}, "auto", options,
                            nullptr, &res);
  res_doc.Parse(res.c_str());
  check_matrix(res_doc, {2.8f, 0.0f}, false, Matrix::CostMatrixAStar);
...
TEST_F(MatrixTrafficTest, CostMatrixWithLiveTraffic) {
  ...
  rapidjson::Document res_doc;
  res_doc.Parse(res.c_str());
  check_matrix(res_doc, {0.0f, 2.8f, 2.8f, 0.0f}, true, Matrix::CostMatrixAStar);
  ASSERT_EQ(result.info().warnings().size(), 0);
  res.erase();
  ...
  res_doc.Parse(res.c_str());
  // the second origin can't respect time (no historical data)
  check_matrix(res_doc, {0.0f, 2.8f, 3.2f, 0.0f}, false, Matrix::CostMatrixAStar);
  ASSERT_EQ(result.info().warnings().size(), 0);
  ...
  result = gurka::do_action(Options::sources_to_targets, map, {"1"}, {"1", "2"}, "auto", options,
                            nullptr, &res);
  res_doc.Parse(res.c_str());
  check_matrix(res_doc, {0.0f, 0.2f}, true, Matrix::CostMatrixAStar);
  ASSERT_EQ(result.info().warnings().size(), 0);
  ...
  result = gurka::do_action(Options::sources_to_targets, map, {"E", "L"}, {"E"}, "auto", options,
                            nullptr, &res);
  res_doc.Parse(res.c_str());
  check_matrix(res_doc, {0.0f, 2.8f}, true, Matrix::CostMatrixAStar);
  ASSERT_EQ(result.info().warnings().size(), 0);
  ...
  check_matrix(res_doc, {0.0f, 2.8f}, false, Matrix::CostMatrixAStar);
  ASSERT_EQ(result.info().warnings().size(), 1);
...
TEST_F(MatrixTrafficTest, TDSources) {
  ...
  result = gurka::do_action(Options::sources_to_targets, map, {"E"}, {"E", "L"}, "auto", options,
                            nullptr, &res);
  res_doc.Parse(res.c_str());
  check_matrix(res_doc, {0.0f, 3.2f}, false, Matrix::CostMatrixAStar);
...
TEST_F(MatrixTrafficTest, TDTargets) {
  ...
  options = {{"/targets/0/date_time", "current"},
             {"/targets/1/date_time", "2016-07-03T08:06"},
             {"/use_costmatrix_astar", "1"}};
  res.erase();
  result = gurka::do_action(Options::sources_to_targets, map, {"E"}, {"E", "L"}, "auto", options,
                            nullptr, &res);
...
TEST(StandAlone, CostMatrixDeadends) {
  ...
  // because of the ABI turn restriction
  {
    auto result = gurka::do_action(valhalla::Options::sources_to_targets, map, {"A"}, {"I"}, "auto",
                                   {{"/use_costmatrix_astar", "1"}}, nullptr, &res);
    res_doc.Parse(res.c_str());
    check_matrix(res_doc, {1.5f}, false, Matrix::CostMatrixAStar);
    res.erase();
  }
  ...
  // then we force to go 1 -> F to hit a blocking node, doing a u-turn and go back the same way
  {
    auto result =
        gurka::do_action(valhalla::Options::sources_to_targets, map, {"1"}, {"B"}, "auto",
                         {{"/sources/0/preferred_side", "opposite"}, {"/use_costmatrix_astar", "1"}},
                         nullptr, &res);
    res_doc.Parse(res.c_str());
    check_matrix(res_doc, {0.8f}, false, Matrix::CostMatrixAStar);
  }
}
...
TEST_F(DateTimeTest, DepartAtCostMatrix) {
  ...
  std::string res;
  {
    ...
    // sanity check
    EXPECT_EQ(api.matrix().algorithm(), Matrix::CostMatrixAStar);
...
TEST(StandAlone, MatrixSecondPass) {
  ...
  {
    auto api = gurka::do_action(valhalla::Options::sources_to_targets, map, {"I", "K"}, {"I", "K"},
                                "auto", {{"/use_costmatrix_astar", "1"}});
