...
TEST_F(Warning, routes) {
  for (auto i : deprecated_costing_methods) {
    valhalla::Api result =
        gurka::do_action(valhalla::Options::route, map, {"A", "C"}, i, {{"/best_paths", "2"}});
...
TEST_F(Warning, locate) {
  for (auto i : deprecated_costing_methods) {
    valhalla::Api result = gurka::do_action(valhalla::Options::locate, map, {"B"}, i);
    ASSERT_TRUE(result.info().warnings_size() != 0);
...
TEST_F(Warning, isochrone) {
  for (auto i : deprecated_costing_methods) {
    valhalla::Api result =
        gurka::do_action(valhalla::Options::isochrone, map, {"B"}, i,
...
TEST_F(Warning, map_matching) {
  for (auto i : deprecated_costing_methods) {
    valhalla::Api trace_route_result =
        gurka::do_action(valhalla::Options::trace_route, map, {"A", "B", "C"}, i);
    valhalla::Api trace_attributes_result =
        gurka::do_action(valhalla::Options::trace_attributes, map, {"A", "B", "C"}, i);
