...
//==========================================================================================
class ExcludeCashOnlyTollWayTest : public ::testing::Test {
protected:
  static gurka::map map;
  ...
  static void SetUpTestSuite() {
    ...
    const gurka::ways ways = {
        {"AB", {{"highway", "primary"}}},
        {"BC", {{"highway", "primary"}, {"toll", "yes"}, {"payment:coins", "yes"}}},
        {"CD", {{"highway", "primary"}}},
        {"DE", {{"highway", "primary"}}},
        {"BF", {{"highway", "primary"}}},
        {"FG", {{"highway", "residential"}, {"surface", "gravel"}}},
        {"GD", {{"highway", "primary"}}},
    };

    const auto layout = gurka::detail::map_to_coordinates(ascii_map, grid_size_meters);
    map = gurka::buildtiles(layout, ways, {}, {}, "test/data/exclude_cash_only_toll_way");
  }
};

gurka::map ExcludeCashOnlyTollWayTest::map = {};

TEST_F(ExcludeCashOnlyTollWayTest, Basic) {
  // Without exclude_cash_only_tolls option
  {
    for (const auto& costing : kSupportedCostingModels) {
      const auto result = gurka::do_action(valhalla::Options::route, map, {"A", "E"}, costing);
