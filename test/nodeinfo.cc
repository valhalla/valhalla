...
void test_ll() {
  ...
  if (!valhalla::midgard::equal<float>(static_cast<Point2>(t.latlng()).DistanceSquared({8, 8}),
                                       125))
  ...
  if (!valhalla::midgard::equal<float>(
          Point2(8, 8).DistanceSquared(static_cast<Point2>(t.latlng())), 125))
    throw std::runtime_error("Distance squared is wrong");
