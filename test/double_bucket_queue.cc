...
void TryRemove(DoubleBucketQueue &dbqueue, size_t num_to_remove, const std::vector<float>& costs)
{
  ...
  for (size_t i = 0; i < num_to_remove; ++i) {
    ...
    test::assert_bool(top != kInvalidLabel, "TryAddRemove: expected " + std::to_string(num_to_remove) + " labels to remove");
...
void TestSimulation()
{
  {
    std::vector<float> costs;
    DoubleBucketQueue dbqueue1(0, 1, 100000, [&costs](const uint32_t label) {
        return costs[label];
      });
  ...
  {
    std::vector<float> costs;
    DoubleBucketQueue dbqueue2(0, 1, 100000, [&costs](const uint32_t label) {
        return costs[label];
      });
  ...
  {
    std::vector<float> costs;
    DoubleBucketQueue dbqueue3(0, 1, 100000, [&costs](const uint32_t label) {
        return costs[label];
      });
  ...
  {
    std::vector<float> costs;
    DoubleBucketQueue dbqueue4(0, 1, 1000, [&costs](const uint32_t label) {
        return costs[label];
      });
