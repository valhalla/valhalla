...
test('optimizedRoute: can return info for locations', function(assert) {
  ...
  valhalla.optimizedRoute(optimizedRouteRequest, (err, result) => {
    ...
    assert.equal(optimizedRoute.trip.legs.length, 3, 'returns three legs');
    assert.equal(optimizedRoute.trip.legs[0].maneuvers.length, 2, 'returns 2 maneuvers');
    assert.equal(optimizedRoute.trip.legs[1].maneuvers.length, 2, 'second leg returns 2 maneuvers');
    assert.equal(optimizedRoute.trip.legs[2].maneuvers.length, 2, 'third leg returns 2 maneuvers');
