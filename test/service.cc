...
namespace {
  const std::list<std::pair<std::string, std::string>> request_responses {
  ...
  void test_failure_requests() {
    ...
    for (const auto& req_resp : request_responses) {
      std::list<zmq::message_t> messages;
      void* request_info;
      messages.emplace_back(zmq::message_t(&req_resp.first[0], req_resp.first.size()));
