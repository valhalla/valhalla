...
TEST(AstarBss, test_With_Mode_Changes) {
  ...
  const std::map<size_t, std::string>& expected_bss_ref{{2, "3006"}, {10, "3008"}};
...
TEST(AstarBss, test_With_Mode_Changes_2) {
  ...
  const std::map<size_t, std::string>& expected_bss_ref{{1, "11103"}, {11, "3008"}};
...
TEST(AstarBss, test_Truck) {
  ...
  std::vector<std::string> expected_route{
      "Rue de la Perle", "Rue des Archives", "Rue Pastourelle",
...
TEST(AstarBss, test_BSSConnections_on_Pedestrian_and_Bicycle) {
  ...
  const std::map<size_t, std::string>& expected_bss_ref{{2, "3006"}, {12, "10011"}};
