...
| `language` | The language of the narration instructions based on the [IETF BCP 47](https://tools.ietf.org/html/bcp47) language tag string. See the [currently supported language list](#supported-language-tags). If no language is specified, the HTTP API will look at the [`Accept-Language`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept-Language) header and attempt to select a supported language using the q-factor weighting in the header. If no header is present, or the specified language(s) are unsupported or invalid, `en-US` is used as the fallback. |
