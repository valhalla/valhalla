...
| `street_side_cutoff` | Cutoff for which road classes the preferred side matters, when `preferred_side` is set to `same` or `opposite`. It should be a string containing one of the following values (from highest to lowest road-class): motorway, trunk, primary, secondary, tertiary, unclassified, residential, service_other. Ignores the preferred side for all road classes which are smaller than the given cutoff class. Default value is `service_other`, so the set preferred side matters for all road classes. |
