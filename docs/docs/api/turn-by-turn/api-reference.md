...
| `service_factor` | A factor that modifies (multiplies) the cost when generic service roads are encountered. The default `service_factor` is 1. |
| `destination_only_penalty` | A penalty applied when entering a road which is only allowed to enter if necessary to reach the destination.
...
| `shape_format` | If the format is `osrm`: Specifies the optional format for the path shape of each connection. One of `polyline6` (default), `polyline5`, `geojson` or `no_shape`. |
