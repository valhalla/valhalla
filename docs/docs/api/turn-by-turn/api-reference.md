...
| `exclude_country_vignettes` |  A set of countries to exclude or avoid within a route can be specified using a JSON array of exclude_country_vignettes under costing_options. If `exclude_country_vignettes` is set for a particular country it is allowed to start and end with the roads containing vignette flag, but is not allowed to have them in the middle of the route path, otherwise they are allowed.|
