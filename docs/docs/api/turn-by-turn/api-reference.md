...
| `exclude_vignettes` | This value indicates whether or not the path may include vignettes. If `exclude_vignettes` is set to true it is allowed to start and end with vignettes, but is not allowed to have them in the middle of the route path, otherwise they are allowed. If set to true, it is highly plausible that no path will be found. Default false. |
