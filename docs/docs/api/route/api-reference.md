...
| `dismount_factor` | A factor that describes how much the cyclist minds getting off the bike and pushing it where riding is not allowed, such as untagged footways, pedestrian ways and crossings or ways tagged `bicycle=dismount`. Pushed sections are always traversed at walking speed; this factor additionally multiplies their cost, so a value of 2 means a minute spent pushing weighs like two minutes riding in the same environment. The range of values is from 1 to 25, where 1 only accounts for the lower speed and 25 makes pushing a last resort. The default value is 2. |
