...
An isochrone request run locally takes the form of `localhost:8002/isochrone?json={}`, where the JSON inputs inside the `{}` includes an array of at least one location and options for the [route costing model](route/api-reference.md#costing-models).
...
Refer to the [route location documentation](route/api-reference.md#locations) for more information on specifying locations.
