...
## Path Computation
  ...
  All of Valhalla’s path algorithms use dynamic, run-time costing. Costing logic is held within the *sif* directory. A base class (*DynamicCost*) defines the interface that each costing model implements.  A brief overview of the costing design is located [here](costing/dynamic-costing.md). 
...
The route narrative/guidance generating code is located in the *odin* directory.
  ...
  - *Odin* supports these [languages](../api/route/api-reference.md#supported-language-tags) for narration instructions
...
## Serializing the Route Response
  ...
  - A more in depth description of the results can be found [here](../api/route/api-reference.md#outputs-of-a-route).
