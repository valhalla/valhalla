...
- `state` ∈ {`merged`, `not-planned`, `superseded`, `won't-fix`, `partial`} for issue entries, **or** `(PR closed)` / `(PR closed, not merged)` when the load-bearing artifact is the closed PR itself. Compound with a structural modifier when relevant, such as `(merged, umbrella)`.
...
- The leading `#N` is wrapped in `**...**` so the Markdown parser does not read it as a block-level heading.
...
- **#6076** (merged) — Keep one canonical agent guide and symlink runtime-specific discovery filenames to it so Claude, Codex, Copilot, Cursor, Gemini, and other coding agents receive the same repository context without duplicated instructions drifting apart. *[tags: agents, documentation, process]*
