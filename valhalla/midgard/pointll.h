...
public:
  ...
  GeoPoint MidPoint(const GeoPoint& p) const {
    ...
    return GeoPoint(atan2(y, x) * -DEG_PER_RAD, atan2(z, sqrt(x * x + y * y)) * DEG_PER_RAD);
  }
   ...
   */
  float Heading(const GeoPoint& ll2) const {
    // If points are the same, return 0
    if (*this == ll2) {
      return 0.0;
    }
   ...
   */
  static float HeadingAlongPolyline(const std::vector<GeoPoint>& pts,
                                    const float dist,
                                    const uint32_t idx0,
                                    const uint32_t idx1) {
   ...
   * @param  dist  Distance in meters from start to find heading to.
   */
  static float HeadingAlongPolyline(const std::vector<GeoPoint>& pts, const float dist) {
   ...
   * @param  idx1  End index within the polyline
   */
  static float HeadingAtEndOfPolyline(const std::vector<GeoPoint>& pts,
                                      const float dist,
                                      const uint32_t idx0,
                                      const uint32_t idx1) {
    // Check that at least 2 points exist
   ...
   *               used to find the heading to the end point.
   */
  static float HeadingAtEndOfPolyline(const std::vector<GeoPoint>& pts, const float dist) {
   ...
   */
  virtual float IsLeft(const GeoPoint& p1, const GeoPoint& p2) const {
   ...
   * @return p  the projected point of this onto the segment uv
   */
  GeoPoint Project(const GeoPoint& u, const GeoPoint& v, float lon_scale) const {
   ...
   *                   Index of the segment of the polyline which contains the closest point >
   */
  std::tuple<GeoPoint, float, int> Project(const std::vector<GeoPoint>& pts) const {
    auto u = pts.begin();
    auto v = pts.begin();
    std::advance(v, 1);
