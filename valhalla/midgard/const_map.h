...
template <size_t Size, class Key, class Value> class ConstFlatMap : public std::array<std::pair<Key, Value>, Size> {
public:
  consteval explicit ConstFlatMap(const std::array<std::pair<Key, Value>, Size>& vals) noexcept {
    ...
    std::sort(this->begin(), this->end(), [](const auto& a, const auto& b) { return a.first < b.first; });
  }
  ...
  [[nodiscard]] constexpr auto find(const Key& key) const {
    auto it =
        std::lower_bound(this->begin(), this->end(), key, [](const auto& a, const auto& b) { return a.first < b; });
