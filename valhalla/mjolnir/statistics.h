...
class statistics {
  ...
  std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, rclassHasher>> country_int_edges;
  ...
  std::unordered_map<std::string, std::unordered_map<RoadClass, float, rclassHasher>> country_truck_route;
  ...
  std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, rclassHasher>> country_axle_load;
...
public:
  ...
  void add_country_int_edge(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);
  ...
  void add_country_speed_info(const std::string& ctry_code, const RoadClass& rclass, const float length);
  ...
  void add_country_truck_route(const std::string& ctry_code, const RoadClass& rclass, const float length);

  void add_tile_height(const uint64_t& tile_id, const RoadClass& rclass, const size_t& count = 1);
  void add_country_height(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);

  void add_tile_width(const uint64_t& tile_id, const RoadClass& rclass, const size_t& count = 1);
  void add_country_width(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);

  void add_tile_length(const uint64_t& tile_id, const RoadClass& rclass, const size_t& count = 1);
  void add_country_length(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);

  void add_tile_weight(const uint64_t& tile_id, const RoadClass& rclass, const size_t& count = 1);
  void add_country_weight(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);

  void add_tile_axle_load(const uint64_t& tile_id, const RoadClass& rclass, const size_t& count = 1);
  void add_country_axle_load(const std::string& ctry_code, const RoadClass& rclass, const size_t& count = 1);
  ...
  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_lengths() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_lengths() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_int_edges() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_int_edges() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_one_way() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_one_way() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_speed_info() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_speed_info() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_named() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_named() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_hazmat() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_hazmat() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_tile_truck_route() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, statistics::rclassHasher>>& get_country_truck_route() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_height() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_height() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_width() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_width() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_length() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_length() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_weight() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_weight() const;

  const std::unordered_map<uint64_t, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_tile_axle_load() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, size_t, statistics::rclassHasher>>& get_country_axle_load() const;
