...
class validator_stats {
  ...
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, float, rclassHasher>> tile_hazmat;
  ...
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, float, rclassHasher>> tile_truck_route;
  ...
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, rclassHasher>> tile_height;
  std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, rclassHasher>> country_height;
  // Count of roads with width
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, rclassHasher>> tile_width;
  std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, rclassHasher>> country_width;
  // Count of roads with length
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, rclassHasher>> tile_length;
  std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, rclassHasher>> country_length;
  // Count of roads with weight
  std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, rclassHasher>> tile_weight;
  std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, rclassHasher>> country_weight;
...
public:
  ...
  void add_tile_hazmat(const uint32_t& tile_id, const RoadClass& rclass, float length);
  void add_country_hazmat(const std::string& ctry_code, const RoadClass& rclass, float length);

  void add_tile_truck_route(const uint32_t& tile_id, const RoadClass& rclass, float length);
  void add_country_truck_route(const std::string& ctry_code, const RoadClass& rclass, float length);

  void add_tile_height(const uint32_t& tile_id, const RoadClass& rclass, const uint32_t& count = 1);
  void add_country_height(const std::string& ctry_code, const RoadClass& rclass, const uint32_t& count = 1);

  void add_tile_width(const uint32_t& tile_id, const RoadClass& rclass, const uint32_t& count = 1);
  void add_country_width(const std::string& ctry_code, const RoadClass& rclass, const uint32_t& count = 1);

  void add_tile_length(const uint32_t& tile_id, const RoadClass& rclass, const uint32_t& count = 1);
  void add_country_length(const std::string& ctry_code, const RoadClass& rclass, const uint32_t& count = 1);

  void add_tile_weight(const uint32_t& tile_id, const RoadClass& rclass, const uint32_t& count = 1);
  void add_country_weight(const std::string& ctry_code, const RoadClass& rclass, const uint32_t& count = 1);

  void add_tile_axle_load(const uint32_t& tile_id, const RoadClass& rclass, const uint32_t& count = 1);
  void add_country_axle_load(const std::string& ctry_code, const RoadClass& rclass, const uint32_t& count = 1);
  ...
  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, float, validator_stats::rclassHasher>>& get_tile_hazmat() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, validator_stats::rclassHasher>>& get_country_hazmat() const;

  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, float, validator_stats::rclassHasher>>& get_tile_truck_route() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, float, validator_stats::rclassHasher>>& get_country_truck_route() const;

  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_tile_height() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_country_height() const;

  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_tile_width() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_country_width() const;

  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_tile_length() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_country_length() const;

  const std::unordered_map<uint32_t, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_tile_weight() const;
  const std::unordered_map<std::string, std::unordered_map<RoadClass, uint32_t, validator_stats::rclassHasher>>& get_country_weight() const;
