...
struct build_stats {
  enum counter : uint8_t {
    ...
    kFailsOSMTimeRange,
    kFailsOSMTimeRangeUnknown,
    kFailsInboundFerryReclass,
    kFailsOutboundFerryReclass,
    kFailsBothFerryReclass,
  ...
  static constexpr meta_entry meta[] = {
      ...
      {"fails_osm_time_range_unknown", "OSM time range causes an unknown runtime_error"},
      {"fails_inbound_ferry_reclass",
       "inbound ferry connections failing to reclassify edges to the next level 0 edge"},
      {"fails_outbound_ferry_reclass",
       "outbound ferry connections failing to reclassify edges to the next level 0 edge"},
      {"fails_both_ferry_reclass",
       "ferry connections completely to reclassify edges to the next level 0 edge"},
      {"exceeded_max_speed", "ways with speed clamped to max"},
      {"exceeded_max_speed_limit", "ways with speed limit clamped to max"},
      {"exceeded_max_truck_speed", "ways with truck speed clamped to max"},
      {"exceeded_max_truck_speed", "SpeedAssigner edges clamped to max"},
      ...
      {"exceeded_max_local_edge_count", "nodes exceeding max density"},
      {"exceeded_max_shape_size", "edges exceeding max encoded shape size"},
      {"exceeded_speed_limit", "edges with speed limit clamped in EdgeInfo"},
      {"elevation_exceeds_diff", "edges with elevation exceeding max difference"},
      {"access_tags_not_found", "edges with access tags not found"},
      {"unrecognized_hov_type", "ways with unrecognized HOV type"},
      {"tag_parse_error", "tag parse errors"},
