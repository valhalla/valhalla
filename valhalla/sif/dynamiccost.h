...
public:
  ...
  virtual float Filter(const baldr::DirectedEdge* edge,
                       const boost::intrusive_ptr<const baldr::GraphTile>& tile) const = 0;
   ...
   */
  virtual void AddToExcludeList(const boost::intrusive_ptr<const baldr::GraphTile>& tile);

  /**
   * Checks if we should exclude or not.
   * @return  Returns true if we should exclude, false if not.
   */
  virtual bool IsExcluded(const boost::intrusive_ptr<const baldr::GraphTile>& tile,
                          const baldr::DirectedEdge* edge);
