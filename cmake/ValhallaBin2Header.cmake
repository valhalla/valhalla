...
Options:
  ...
  if(NOT exit)
    ...
    elseif(conversion_type MATCHES LOCALES)
      file(WRITE ${target} "#include <string_view>\n#include \"midgard/const_map.h\"\n")
      file(GLOB json_files LIST_DIRECTORIES FALSE "${source}/*.json")

      # Embed each locale JSON as a raw string literal
      foreach(file ${json_files})
        get_filename_component(name "${file}" NAME)
        string(MAKE_C_IDENTIFIER "${name}" name)
        string(TOLOWER "${name}" name)
        file(READ ${file} content)
        file(APPEND ${target} "constexpr std::string_view ${name} = R\"L0CAL3(${content})L0CAL3\";\n\n")
      endforeach()

      # Build constexpr ConstFlatMap
      set(map "constexpr std::pair<std::string_view, std::string_view> locales_json_data[] = {\n")
      foreach(file ${json_files})
        get_filename_component(name "${file}" NAME)
        get_filename_component(locale "${file}" NAME_WE)
        string(MAKE_C_IDENTIFIER "${name}" name)
        string(TOLOWER "${name}" name)
        set(map "${map}    {\"${locale}\", ${name}},\n")
      endforeach()
      set(map "${map}};\ninline constexpr auto locales_json = valhalla::midgard::ConstFlatMap(locales_json_data);\n")
