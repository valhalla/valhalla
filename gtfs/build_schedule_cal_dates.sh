...
if [[ "$2" ==  "pg" ]]; then
  dbuser=$3
  psql -U $dbuser -c "copy (select stop_key,trip_key,trip_id,route_key,service_key,shape_key,departure_time,arrival_time,start_date,end_date,has_subtractions,block_id,headsign,wheelchair_accessible,bikes_allowed from s_dates_tmp order by trip_id,start_date, stop_sequence) to STDOUT csv" $db > ./schedule.tmp
elif [[ "$2" == "sqlite" ]]; then 
  sqlite3 $db -csv "select stop_key,trip_key,trip_id,route_key,service_key,shape_key,departure_time,arrival_time,start_date,end_date,has_subtractions,block_id,headsign,wheelchair_accessible,bikes_allowed from s_dates_tmp order by trip_id,start_date, stop_sequence;" > ./schedule.tmp
fi
